...
{
    public sealed class RevitViewsToDwgComponent : BaseComponent
    {
        private bool wasTriggered;

        public RevitViewsToDwgComponent()
            : base(
                "Revit Views to DWG",
                "ViewsDWG",
                "Export one or more Revit views to DWG on a rising-edge Run trigger.",
        ...
        {
            get { return new Guid("fec1cad0-2371-4c43-83ef-2d9f3bc83434"); }
        }

        protected override void RegisterInputParams(GH_InputParamManager parameterManager)
        {
            parameterManager.AddGenericParameter("Views", "V", "Revit views to export.", GH_ParamAccess.list);
            ...
            parameterManager.AddBooleanParameter("Run", "Run", "Run the DWG export on a rising-edge trigger.", GH_ParamAccess.item, false);
        ...
        protected override void RegisterOutputParams(GH_OutputParamManager parameterManager)
        {
            ...
            parameterManager.AddTextParameter("Paths", "P", "Exported DWG paths.", GH_ParamAccess.list);
        ...
        protected override void SolveInstanceCore(IGH_DataAccess dataAccess)
        {
            ...
            if (!run)
            {
                dataAccess.SetData(0, "Ready. Set Run to true to export the supplied views to DWG.");
                wasTriggered = false;
                return;
            }
            ...
            {
                Result<string> exportResult = RevitViewExportOperations.ExportDwg(document, targetResult.Value[index].View, targetResult.Value[index].Path);
            ...
            string summary = "Exported " + exportedPaths.Count + " of " + targetResult.Value.Count + " view(s) to DWG.";
            if (errors.Count > 0)
            {
